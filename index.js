...
const app=express();
app.use(cors({
    origin:['http://localhost:5173','https://6525362d9f369d16f9b12908--beamish-tiramisu-dce7d4.netlify.app'],
    credentials:true
}));
...
app.get('/google/auth',async(req,res)=>{
    ...
    res.cookie('token',storedAccessToken,{
     ...
     res.redirect('https://6525362d9f369d16f9b12908--beamish-tiramisu-dce7d4.netlify.app/main');
});
...
app.get('/logout',async(req,res)=>{
  console.log("req is made on this logout route");
    const token=req.cookies.token;
  if(!token||token==''){
        return  res.redirect("https://6525362d9f369d16f9b12908--beamish-tiramisu-dce7d4.netlify.app");
    }
    console.log(token);
    const val=await tDB.find({access_token:token});
    if(val.length==0||!val){
